...
if [ -z != ${10} ]; then
	...
	if [ -z != ${11} ]; then
		...
		echo "${11}" > $HOME/shell.sh
		echo "exit" >> $HOME/shell.sh
		cat $HOME/shell.sh
		apk add sshpass
		sshpass -p ${10} ssh -o StrictHostKeyChecking=no -p $3 $1@$2 < $HOME/shell.sh
		
		echo 'Service Commands Executed. Please Check Server Logs'
	fi
    exit 0
fi
...
if [ -z != ${11} ]; then
	...
	echo "${11}" > $HOME/shell.sh
  	echo "exit" >> $HOME/shell.sh
  	cat $HOME/shell.sh
	ssh -o StrictHostKeyChecking=no -p $3 -i $TEMP_SSH_PRIVATE_KEY_FILE $1@$2 < $HOME/shell.sh
